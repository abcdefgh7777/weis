<template>
  <div class="index-page-box">
    ...
    >
      <div class="title-row">
        <h1><span class="text-bg">WEI</span></h1>
        <a
          href="https://x.com/weicli0x"
          target="_blank"
          class="x-link"
          @click.stop
        >
          <img src="/image/logo/x-logo.svg" alt="X" class="x-icon" />
        </a>
      </div>
...
onBeforeUnmount(() => {
  // Destroy the previously created Experience object
  if (window.experience) {
...
.info-box {
  ...
  .x-icon {
    @apply w-[max(1.2rem,3vmin)] h-[max(1.2rem,3vmin)];
    filter: invert(1);
  }
