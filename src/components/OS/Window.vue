        ...
        <keep-alive>
          ...
          >
            <component
              ...
              @onDrag="onDrag($event)"
              @onMax="onMax"
              @onClose="onClose(setting.id)"
              @onMin="onMin(setting.id)"
            />
          </div>
        </keep-alive>
      </div>
...
// Dynamically load APP component
// TODO: vue defineAsyncComponent not working here, using workaround
...
import OSAPPtradingLog from "./APP/tradingLog.vue";
...
/**
 * Resize window
 * @param {*} direction direction string top|left|right|bottom
 */
